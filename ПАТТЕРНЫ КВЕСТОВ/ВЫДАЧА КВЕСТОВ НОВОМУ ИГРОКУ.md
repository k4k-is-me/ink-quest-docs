...
> ⚠️ Это вызовет функцию **повторно** — если в ней есть `quest give`, повторная выдача активного квеста выдаст ошибку `already_tracked`. Чтобы повтор был безопасным — либо проверяй состояние перед выдачей, либо делай новые квесты [[ПОВТОРЯЕМЫЙ КВЕСТ|repeatable]].
...
1. `story:on_join` — выдаёт «нейтральный» стартовый набор и [[РАЗВИЛКА И ПАМЯТЬ ВЫБОРА|квест выбора]].
...
- [[ЦЕПОЧКА КВЕСТОВ]] — после стартового квеста зависимые откроются автоматически
- [[РАЗВИЛКА И ПАМЯТЬ ВЫБОРА]] — если стартовый квест предлагает выбор класса/фракции
- [[ГЛОБАЛЬНЫЙ КВЕСТ]] — если стартовый квест включает игрока в текущий серверный ивент
- [[ХУКИ ЖИЗНЕННОГО ЦИКЛА ЗАДАЧИ]] — функции и теги в `on.load`/`on.success` тоже могут готовить игрока
