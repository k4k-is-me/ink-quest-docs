...
`condition.success` смотрит на scoreboard виртуального игрока `GLOBAL`. Каждый участник через `on.tick` переносит свой личный вклад в этот общий счёт. Когда `GLOBAL` достигает цели — `success` срабатывает у каждого участника независимо.
...
- `"player": "GLOBAL"` — счёт виртуального игрока. Имя любое (`SERVER`, `EVENT_KARMA`, `тьма`), но соглашение `GLOBAL` принято в этой документации.
- Поле `initial` **не указано** — это критично, см. подводные камни ниже.
...
### ⚠️ Не задавай `initial` для общего счётчика

При загрузке задачи поле `initial` записывает значение в scoreboard. А поскольку задача загружается у **каждого** участника, накопленный прогресс будет **обнуляться** всякий раз, когда новый игрок получает квест или заходит на сервер.
...
✅ **Правильно:** не указывай `initial`, а если нужно гарантировать стартовое значение — поставь его один раз в функции мира:

```mcfunction
# data/legion_fall/functions/world_init.mcfunction (в minecraft:load)
execute unless score GLOBAL zombies_killed_total matches 0.. run scoreboard players set GLOBAL zombies_killed_total 0
```

`initial` имеет смысл только для **индивидуальных** счётчиков (когда каждый игрок начинает с одного значения — например, личный таймер).
...
Два виртуальных игрока (`FACTION_RED`, `FACTION_BLUE`), два условия `score`:
