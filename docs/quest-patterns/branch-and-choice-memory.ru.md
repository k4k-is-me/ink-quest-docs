...
InkQuest строит ветвление без специальных полей. Хватает трёх примитивов: условие `tasks`, опциональные задачи и scoreboard-теги. Вместе они образуют полноценный state-machine.
...
Этап содержит **обязательную задачу с условием `tasks`** и несколько **опциональных задач**. Игрок выполняет одну опциональную — обязательная закрывается сама, этап считается пройденным.
...
- **Обязательная `speak`** — это «детектор выбора». Условие `tasks` без поля `tasks` смотрит на задачи **активного этапа, кроме самой себя** — то есть автоматически на все три опциональные. `count: 1` означает «достаточно одной с успехом».
...
"speak": {
  ...
  "condition": {
    "success": { "type": "tasks", "status": "success", "count": 3 }
  }
}
```
...
Теперь кнопка под «Соврать» — `Failure`. Клик → задача завершается `failure` → квест немедленно уйдёт в `failure` (поскольку условие `tasks` ловит любой терминальный статус по умолчанию, но если хотим именно провал — добавь `"status": "failure"` в условии `speak`). См. [ЗАДАЧА С ДВУМЯ ИСХОДАМИ](/quest-patterns/task-with-two-outcomes.md) про эту логику.
