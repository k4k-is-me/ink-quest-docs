...
Или, что удобнее для записок, — через [свиток квеста](/player-view/items.md). Игрок поднимает свиток (физически — это записка), ПКМ выдаёт ему квест и сразу открывает в книге.
...
Если по одной локации разбросана серия записок («Дневник старого траппера, часть 1», «…часть 2», «…часть 3») — заведи на каждую отдельный квест-заметку и свяжи их через [Цепочка квестов](/quest-patterns/quest-chain.md). Каждая следующая часть появится в книге только после прочтения предыдущей.
...
После прочтения у игрока появится scoreboard-тег `story.read_fathers_letter`. Дальнейшие квесты могут проверить этот тег через `require.tags` и открыться только тем, кто читал письмо — см. [Развилка и память выбора](/quest-patterns/branch-and-choice-memory.md).
...
- [Структура квеста](/how-quests-work/quest-structure.md) — про квесты без этапов
- [Предметы](/player-view/items.md) — Quest Scroll, самый удобный способ раздавать заметки
- [Цепочка квестов](/quest-patterns/quest-chain.md) — если заметок несколько и они идут серией
- [Развилка и память выбора](/quest-patterns/branch-and-choice-memory.md) — если факт прочтения должен влиять на дальнейшее
