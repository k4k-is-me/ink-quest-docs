...
Теги — это «память» исхода: последующие квесты могут открыться только для тех, кто доставил (или, наоборот, провалил) задание. См. [РАЗВИЛКА И ПАМЯТЬ ВЫБОРА](/quest-patterns/branch-and-choice-memory.md).
...
- `target: 0` — условие сработает, когда счёт **опустится** до 0. Поскольку `initial > target`, условие читается как нисходящее (см. [УСЛОВИЯ ВЫПОЛНЕНИЯ ЗАДАЧИ](/datapack-reference/task-completion-conditions.md)).
...
- [УСЛОВИЯ ВЫПОЛНЕНИЯ ЗАДАЧИ](/datapack-reference/task-completion-conditions.md) — полная документация по типам условий
- [ХУКИ ЖИЗНЕННОГО ЦИКЛА ЗАДАЧИ](/datapack-reference/task-lifecycle-hooks.md) — про `on.success`, `on.failure`, `on.tick`
- [СТАТУСЫ КВЕСТОВ И ЗАДАЧ](/how-quests-work/quest-and-task-statuses.md) — как `failure` обязательной задачи влияет на итог квеста
- [РАЗВИЛКА И ПАМЯТЬ ВЫБОРА](/quest-patterns/branch-and-choice-memory.md) — как зафиксировать исход в теге и развести сюжет
