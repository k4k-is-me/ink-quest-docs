...
| [`optionals`](#условие-по-статусу-optional-задач-optionals) | Статусы optional-задач активного этапа         | Прогресс-бар (`k / N`)           |
...
Если пул пуст (других optional-задач нет) — условие выполнено сразу. Условие всегда отображается как прогресс-бар `k / N` в HUD: где `k` — число совпадающих по статусу задач, `N` — `min` или размер пула.
