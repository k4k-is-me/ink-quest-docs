...
**Засчитывается только успех.** Зависимость открывает группу, лишь если завершилась исходом `success`. Квест, завершённый провалом (`failure`) или пропуском (`skipped`), группу не открывает. Что это за исходы — в [Статусы квестов и задач](/how-quests-work/quest-and-task-statuses.md#три-исхода).
